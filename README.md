...
В этом проекте тебе и другим участником предстоит обьединиться в команду, а затем вместе спроектировать и разработать веб-приложение, которое помогает 
прогнозировать риск инсульта и предлагает рекомендации по его снижению.

## Содержание

1. [Chapter I](#chapter-i)  
   - [Рекомендации к проекту](#рекомендации-к-проекту)
   - [Рекомендации по командоной работе ](#рекомендации-по-командоной-работе)
...
-	Используйте канбан-доску: это поможет отслеживать прогресс и видеть, на чем работают другие. Например, воспользуйтесь GoKanban.io.
...
Итог работы — текстовая схема доски, сохраненная в файле kanban.txt.
Помните, что доска — это живой инструмент: задачи будут добавляться,
изменяться и уточняться по мере работы. Не пытайтесь распланировать всё наперед — лучше подходить к процессу итеративно, адаптируясь к новым задачам и вызовам.

Дальнейшие задания предложены в логической последовательности,
но вы можете выполнять их в удобном для вашей команды порядке.

Для совместного написания кода проекта рекомендуется использовать git и платформу для хранения репозитория (GitHub/Gitlab/...). Вести историю проекта в git и вносить изменения от участников с помощью Pull Request/ Merge Request (в зависисмости от платформы)

## Chapter II

### Задание 1: Поиск данных

Наша цель — создать приложение, которое поможет пользователям определить риск инсульта и, при необходимости, предложит рекомендации по его снижению. Для этого понадобится предиктивная модель, способная прогнозировать риск на основе введенных пользователем данных.
...
Основная задача ML-инженера — создать предиктивную модель,
 которая точно прогнозирует риск инсульта. 
 Для этого нужно провести тщательный анализ данных, 
 выделить ключевые признаки и выбрать наилучший алгоритм машинного обучения.

Что нужно сделать:

1.	Работа с данными в Jupyter Notebook
	-	Проведи все эксперименты и предварительную работу в ноутбуке, чтобы было удобно анализировать и документировать процесс.
2.	Анализ данных
	-	Изучи доступные признаки: выясни, какие из них наиболее значимы для прогнозирования инсульта.
	-	Убери ненужные или нерелевантные данные, чтобы улучшить качество модели.
	-	Приведи данные к чистому и структурированному виду.
3.	Обучение модели
	-	Обучи несколько моделей машинного обучения (например, Random Forest, Gradient Boosting, Logistic Regression).
	-	Сравни результаты на тестовой выборке с помощью метрик (например, ROC-AUC, Precision, Recall) и выбери модель с лучшим качеством.
4.	Код для работы с моделью. Напиши код для:
...
5.	Классификация риска
	-	Добавь функционал для классификации риска на категории: 
...
Этап работы над моделью — это основа всего приложения. Убедись, что модель демонстрирует хорошее качество и позволяет точно прогнозировать риск инсульта!


### Задание 3: Разработка пользовательского интерфейса

Один из ключевых элементов приложения — это его внешний вид и способ взаимодействия с пользователем. В этом задании участникам необходимо спроектировать и реализовать интерфейс,
через который пользователь сможет взаимодействовать с системой.
...
Любое приложение нуждается в надежной базе данных для хранения пользовательских данных,
и ваше — не исключение. Задача backend-инженеров — спроектировать и настроить базу данных,
которая будет эффективно поддерживать работу приложения, включая хранение введенных
данных и результатов прогнозов.
...
Рекомендации:
В зависимости от загрузки и погруженности в код участников разбейте это задания на задачи и распределите их по команде.
...
1.	Ручная логика рекомендаций
	...
	-	На основе введенных данных пользователя анализируйте ключевые факторы риска (например, высокий уровень глюкозы, малоподвижный образ жизни) и предлагайте соответствующие рекомендации.
...
1.	Поанализируйте оба подхода, их трудозатраты, дающие эффекты и риски. Определите, какой способ реализации рекомендаций подходит вашему проекту.
...
Рекомендации:
В качстве LLM можно использовать api gigachat, либо самостоятельно поднять и развернуть одну из современных языковых моделей, например LLaMA.
...
3. Создание документации.
	- Разработайте техническую документацию и разместите её в файле README.md:
