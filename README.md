...
-	Используйте канбан-доску: это поможет отслеживать прогресс и видеть, над чем работают другие. Например, воспользуйтесь GoKanban.io.
...
Итогом работы должна стать текстовая схема доски, сохраненная в файле `kanban.txt`. Помните, что доска — это живой инструмент: задачи будут добавляться, изменяться и уточняться по мере работы. Не пытайтесь распланировать всё наперед — лучше подходить к процессу итеративно, адаптируясь к новым задачам и вызовам.
